module Boukensha
  ...
  class Repl
    COMMANDS = %w[/clear /compact /exit /quit /help].freeze

    def initialize(context:, registry:, builder:, client:, logger: NullLogger.new,
                   max_iterations: Agent::MAX_ITERATIONS, max_turn_tokens: nil, max_output_tokens: 1024,
      ...
      @agent = Agent.new(
        ...
        max_iterations: max_iterations, max_turn_tokens: max_turn_tokens, max_output_tokens: max_output_tokens
      )
    end
    ...
    def handle_command(line)
      ...
      when "/compact"
        ...
        @output.puts "Dropped #{dropped} message#{'s' unless dropped == 1}."
        nil
      when "/exit", "/quit"
        @output.puts "Goodbye."
        :exit
