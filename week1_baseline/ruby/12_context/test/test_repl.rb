...
class TestRepl < Minitest::Test
  ...
  def test_compact_command_drops_messages_and_reports_the_count
    repl, _client, output = build_repl([], input_lines: ["/compact", "/exit"])
    10.times { |i| @context.messages << Boukensha::Message.new(:user, "msg #{i}") }
    @context.update_tokens(1)
  ...
  def test_compact_command_does_not_call_the_client
    repl, client, = build_repl([], input_lines: ["/compact", "/exit"])
    @context.messages << Boukensha::Message.new(:user, "a")
    @context.messages << Boukensha::Message.new(:user, "b")
    @context.messages << Boukensha::Message.new(:user, "c")
